...
class Challenge_Controller extends Auth_Controller
{
    public function indexAction($data=NULL){
        ...
        $data['list_challenge'] = get_all_by_tablename($Database,'challenge');
        if ($data['list_challenge'] !== false){
        $number_challenges = count($data['list_challenge']);
            for ($i=0; $i < $number_challenges;$i++){
                $data['list_challenge'][$i]['creator'] = get_by_id($Database,'user',$data['list_challenge'][$i]['creator_id'])['fullname'];
            }
        }
        $this->load_header('header',$data);
        $this->view->load('challenge',$data);
